...
from utils.functions_utils import distribution_function as df
from utils.functions_utils import euclidean_function as ef
...
class GaussianMixtureSKLearn(skm.GaussianMixture):
    def __init__(self, n_gaussian, init_mod="rand", verbose=True, covariance_type="full"):
        ...
        super(GaussianMixtureSKLearn, self).__init__(
            n_components=n_gaussian, covariance_type=covariance_type, max_iter=300)
    ...
    def fit(self, X, Y=None):
        ...
        if (Y is not None):
            ...
            means = ((X.unsqueeze(1).expand(N, M, D) * Y.unsqueeze(-1).expand(N,
                     M, D)).sum(0)/Y.sum(0).unsqueeze(-1).expand(M, D))
            self.means_2 = means.numpy()
            # print("mean -> ", self.means_2.shape)
            # print("mean[0] -> ", self.means_2[:,0])
            self.N_k = Y.sum(0)
            XmM = X.unsqueeze(1).expand(N, M, D) - \
                means.unsqueeze(0).expand(N, M, D)
            # print((XmM * XmM).sum(-1))
            self.covariances_2 = (((XmM * XmM).sum(-1)) * Y).sum(0)/Y.sum(0)/30
            # print((XmM * XmM).sum(-1).mean())
            # print("cov ", self.covariances_2)
            super(GaussianMixtureSKLearn, self).__init__(n_components=self.n_gaussian, covariance_type="spherical", precisions_init=(
                1/self.covariances_2).numpy(), weights_init=self.weights_/self.weights_.sum(), means_init=self.means_2, max_iter=5)
...
class GMM(object):
    ...
    def update_mu(self, z, wik, lr_mu, tau_mu, g_index=-1, max_iter=50):
        ...
        if (g_index > 0):
            self._mu[g_index] = (
                wik[:, g_index].unsqueeze(-1).expand(N, D) * z).sum(0)/wik[:, g_index].sum()
        else:
            self._mu = (wik.unsqueeze(-1).expand(N, M, D) * z.unsqueeze(1).expand(N,
                        M, D)).sum(0)/wik.sum(0).unsqueeze(-1).expand(M, D)
    ...
    def update_sigma(self, z, wik, g_index=-1):
        ...
        N_k = wik.sum(0)
        if (g_index > 0):
            self._sigma[:, g_index] = ((self._distance(z, self._mu[:, g_index].expand(
                N))**2) * wik[:, g_index]).sum()/wik[:, g_index].sum()
        else:
            dtm = self._distance(z.unsqueeze(1).expand(
                N, M, D), self._mu.unsqueeze(0).expand(N, M, D))
            ZmMU = z.unsqueeze(1).expand(N, M, D) - \
                self._mu.unsqueeze(0).expand(N, M, D)
            sigma = []
            ...
            if (self.mod == "full"):
                for i in range(M):
                    ...
                    wik_k = wik_k[index_nz].unsqueeze(
                        -1).unsqueeze(-1).expand(n_nz, D, D).double()
                    ZmMU_k = ZmMU_k[index_nz].unsqueeze(1).double()
                    # print(ZmMU_k.size())
                    ZmMU_k_dot = (
                        ZmMU_k.transpose(-1, 1).bmm(ZmMU_k) * wik_k).sum(0)
                    sigma.append(
                        (ZmMU_k_dot/(wik[:, i].sum().double())).unsqueeze(0))
                self._sigma = torch.cat(sigma, 0)
            elif (self.mod == "diag"):
                g = (ZmMU**2 * wik.unsqueeze(-1).expand(N, M, D)).sum(0) / \
                    wik.unsqueeze(-1).expand(N, M, D).sum(0)
                self._sigma = g.unsqueeze(-1).expand(M, D, D) * \
                    torch.eye(D).unsqueeze(0).expand(M, D, D)
            elif (self.mod == "spherical"):
                g = ((ZmMU**2).sum(-1) * wik.expand(N, M)
                     ).sum(0)/wik.expand(N, M).sum(0)
                self._sigma = g.unsqueeze(-1).unsqueeze(-1).expand(
                    M, D, D) * torch.eye(D).unsqueeze(0).expand(M, D, D)
    ...
    def get_pik(self, z):
        ...
        pdf = df.gaussianPDF(z, self._mu, self._sigma,
                             norm_func=self.norm_ff, distance=self._distance)
    ...
    def log_probs_cholesky(self, z):
        ...
        for i in range(self._mu.size(0)):
            try:
                ...
                eigen_value, eigen_vector = torch.symeig(
                    self._sigma[i], eigenvectors=True)
        ...
        dtm = z.unsqueeze(1).expand(N, M, D) - \
            self._mu.unsqueeze(0).expand(N, M, D)
        ...
        for i in range(M):
            ...
            exp_dist = dtm_k.bmm(inv_sig_k).bmm(
                dtm_k.transpose(-1, 1)).squeeze(-1)

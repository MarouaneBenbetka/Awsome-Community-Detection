...
class PoincareKMeansNInit(object):
    def __init__(self, n_clusters, min_cluster_size=5, verbose=False, init_method="kmeans++", n_init=20):
        ...
        self.KMeans = [PoincareKMeans(
            n_clusters, min_cluster_size, verbose, init_method) for i in range(n_init)]
...
class PoincareKMeans(object):
    def __init__(self, n_clusters, min_cluster_size=2, verbose=False, init_method="kmeans++", random_seed=None):
        self._init_methods_set = {
            'random': _init_random, 'kmeans++': _init_kmeansPP}
    ...
    def _fast_maximisation(self, x, indexes, batch_size=10):
        ...
        nb_batch = (self._n_c // batch_size) + \
            (1 if ((self._n_c % batch_size) != 0) else 0)
    ...
    def fit(self, X, initial_seed_index=None, Y=None, max_iter=100):
        lt = []
        ft = 0
        if initial_seed_index is not None:
            # Use the provided initial seed index
            self.centroids = X[initial_seed_index]
            self.indexes = self._expectation(self.centroids, X)
            self.centroids = self._maximisation(X, self.indexes)
        if Y is None:
            # Clustering without initial labels
            with torch.no_grad():
                if self._mec < 0:
                    self._mec = len(X) / (self._n_c ** 2)
                if self.centroids is None:
                    centroids_index, centroids_value = self._init_method(
                        X, self._n_c, self._distance)
            ...
            # Clustering with initial labels
            self.indexes = Y  # Use provided labels
            self.centroids = self._maximisation(X, self.indexes)
            self.cluster_centers_ = self.centroids
            return self.centroids
...
def _test_init_kmeans_pp():
    ...
    plt.scatter(centroids_value[:, 0].numpy(
    ), centroids_value[:, 1].numpy(), label="initial centroids")

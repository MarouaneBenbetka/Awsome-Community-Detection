from utils.manifold.manifold import Manifold
from utils.manifold.tools import gyration
from utils.functions_utils import function
...
class PoincareBallApproximation(PoincareBall):
    ...
    class PoincareDistance(torch.autograd.Function):
        ...
        def backward(ctx, g):
            ...
            g = g.unsqueeze(-1)
            gu = PoincareBallApproximation.PoincareDistance.grad(
                u, v, squnorm, sqvnorm, sqdist, ctx.eps)
            gv = PoincareBallApproximation.PoincareDistance.grad(
                v, u, sqvnorm, squnorm, sqdist, ctx.eps)
    ...
    def barycenter(z, lr=1e-1, tau=5e-4, max_iter=math.inf):
        with torch.no_grad():
            ...
            while (cvg > tau and max_iter > iteration):
                iteration += 1
                grad_tangent = 2 * \
                    PoincareBallApproximation.riemannian_log(
                        barycenter.expand_as(z), z)
                grad_tangent /= len(z)
                cc_barycenter = PoincareBallApproximation.riemannian_exp(
                    barycenter, lr * grad_tangent.sum(0, keepdim=True))
                cvg = PoincareBallApproximation.distance(
                    cc_barycenter, barycenter).max().item()
    ...
    def riemannian_exp(x, k):
        ...
        if (outter_disc.shape[0] >= 1):
            exp[outter_disc] =\
                torch.einsum(
                    'ij,i...->ij', exp[outter_disc], 1 / (norm[outter_disc] + 1e-5))
        return exp
    ...
    def barycenter(z, lr=1e-1, tau=5e-4, max_iter=math.inf):
        with torch.no_grad():
            ...
            while (cvg > tau and max_iter > iteration):
                iteration += 1
                grad_tangent = 2 * \
                    PoincareBallExact.riemannian_log(
                        barycenter.expand_as(z), z)
                grad_tangent /= len(z)
                cc_barycenter = PoincareBallExact.riemannian_exp(
                    barycenter, lr * grad_tangent.sum(0, keepdim=True))
                cvg = PoincareBallExact.distance(
                    cc_barycenter, barycenter).max().item()
                barycenter = cc_barycenter
            return barycenter
...
def test_log_gradient_distance():
    ...
    for i in range(10):
        ...
        decomposed_grad = PoincareBallExact.euclidean_to_riemannian_grad(
            x, x.grad)
        log_grad = - \
            PoincareBallExact.riemannian_log(
                x, y)/PoincareBallExact.distance(x, y)

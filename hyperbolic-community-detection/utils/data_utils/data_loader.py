...
DATA_INF = {
    ...
    "karate": {
        "graph": {"filepath": "data/reel/karate/karate.gml", "filetype": "gml"},
        "communities": {"filepath": "data/reel/karate/groundTruth.txt", "filetype": "txt"},
        "directed": False
    },
    "books": {
    ...
    "football": {
        "graph": {"filepath": "data/reel/football/football.gml", "filetype": "gml"},
        "communities": {"filepath": "data/reel/football/groundTruth.txt", "filetype": "txt"},
        "directed": False
    },
    "dolphins": {
        "graph": {"filepath": "data/reel/dolphins/dolphins.gml", "filetype": "gml"},
        "communities": {"filepath": "data/reel/dolphins/groundTruth.txt", "filetype": "txt"},
        "directed": False
    },
    "polbooks": {
        "graph": {"filepath": "DATA/reel/polbooks/polbooks.gml", "filetype": "gml"},
        "communities": {"filepath": "data/reel/polbooks/groundTruth.txt", "filetype": "txt"},
        "directed": False
    },
    "email": {
        "graph": {"filepath": "DATA/reel/email/email.dat", "filetype": "dat"},
        "communities": {"filepath": "data/reel/email/groundTruth.txt", "filetype": "txt"},
        "directed": False
    },
    "adjnoun": {
...
def load_corpus(corpus_id, directed=None):
    ...
    X = LOAD_INF["graph"][corpus_infos["graph"]["filetype"]](
        corpus_infos["graph"]["filepath"])
    if (not directed):
        X = directed_to_undirected(X)
    Y = LOAD_INF["communities"][corpus_infos["communities"]
                                ["filetype"]](corpus_infos["communities"]["filepath"])
...
def read_txt_communities(communities_filepath):
    ...
    min_y_label = float('inf')
    with io.open(communities_filepath, "r") as label_file:
        for i, line in enumerate(label_file):

            Y[i] = []
            Y[i].append(int(line))
            min_y_label = min(min_y_label, int(line))

    # make the labels 0-indexed
    for y in Y:
        Y[y] = [y_label - min_y_label for y_label in Y[y]]

    return Y


def read_gml_graph(graph_filepath):
    """ 
    Loading edges and nodes of a graph, where graph is represented
    with the gml format
    ...
    X : a dictionary having for keys nodes and values list of neigbhor nodes
    """
    G = nx.read_gml(graph_filepath, label='id')

    # make the nodes 0 indexed
    ...
    X = {}
    for node in G.nodes():
        X[node] = [k for k in G.neighbors(node)]
    return X


def read_dat_graph(graph_file):
    """ 
    Loading edges and nodes of a graph, where graph is represented
    with the dat format
    ...
    Parameters: 
    graph_file (str): The filepath of the graph file

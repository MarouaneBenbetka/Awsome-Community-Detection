...
class EvaluationMetrics(object):
    ...
    def macro_precision(self):
        precision_by_label = [
            tp/(tp+fp) if (tp+fp != 0) else 0 for tp, fp in zip(self.TP, self.FP)]

        return sum(precision_by_label, 0)/(len(precision_by_label))

    def macro_recall(self):
        recall_by_label = [tp/(tp+fn) if (tp+fn != 0)
                           else 0 for tp, fn in zip(self.TP, self.FN)]
...
def precision_at(prediction, ground_truth, at=5):
    ...
    trange = torch.arange(
        len(prediction)).unsqueeze(-1).expand(len(prediction), at).flatten()
    indexes = prediction_index[:, :at].flatten()
    score = ((ground_truth[trange, indexes]).float().view(
        len(prediction), at)).sum(-1)/at
...
def nmi(prediction, ground_truth):
    return normalized_mutual_info_score(prediction, [y[0] for y in ground_truth.values()])
...
def intra_cluster_density(annotation, adjency_matrix, weighted=False):
    ...
    for cluster_index in range(K):
        ...
        cluster_nodes = annotation[:,
                                   cluster_index].nonzero().flatten().tolist()
        ...
        if (cluster_size > 1):
            ...
            cluster_score = cluster_kappa_intra / \
                (0.5 * cluster_size * (cluster_size - 1))
            if (weighted):
...
class CrossValEvaluation(object):
    def __init__(self, embeddings, ground_truth, nb_set=5, algs_object=PoincareEM):
        ...
        self.subset_indexer = [
            subset_index[nb_value * i:min(nb_value * (i+1), len(self.z))] for i in range(nb_set)]
        self.all_algs = []
        ...
        for i, test_index in zip(pb, self.subset_indexer):
            ...
            train_index = torch.cat(
                [subset for ci, subset in enumerate(self.subset_indexer) if (i != ci)], 0)
...
def precision1_unsupervised_evaluation(cluster_prediction, ground_truth, n_distrib):
    ...
    print("min gt label estimation ", torch.LongTensor(
        [ground_truth[i][0] for i in range(len(ground_truth))]).min())
    labels_matrix = torch.zeros(n_distrib, n_distrib).cuda()
    cluster_prediction = cluster_prediction.cuda()
    import tqdm
    gt_matrix = [torch.LongTensor(ground_truth[i]).cuda()

...
class RandomWalkCorpus(Dataset):
    ...
    def _walk(self, index):
        ...
        for i in range(self.k):
            ...
            c_index = self.X[c_index][random.randint(
                0, len(self.X[c_index])-1)]
            path.append(c_index)
...
class FlatContextCorpus(Dataset):
    def __init__(self, dataset, context_size=5, precompute=1):
        ...
        if (precompute < 1):
            print("Precompute is mandatory value "+str(precompute) +
...
class RandomContextSize(RandomWalkCorpus):
    ...
    def _context_calc(k, path, max_context):
        ...
        v = torch.cat((path[max(0, k - context_size):k],
                      path[k+1: min(len(path), k + context_size)]), 0)
    ...
    def __getitem__(self, index):
        ...
        res = [RandomContextSize._context_calc(
            i, path, self.c_s) for i in range(len(path))]

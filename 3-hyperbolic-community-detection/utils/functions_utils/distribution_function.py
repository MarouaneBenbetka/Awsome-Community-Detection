...
def weighted_gmm_pdf(w, z, mu, sigma, distance, norm_func=None):
    ...
    if (norm_func is None):
        zeta_sigma = pi_2_3 * sigma * \
            torch.exp((sigma**2/2) * erf_approx(sigma/math.sqrt(2)))
    else:
...
def log_grad_zeta(x, N):
    ...
    if (binomial_coefficient is None):
        ...
        v[0] = 1
        n_fact = v.prod()
        k_fact = torch.cat([v[:i].prod().unsqueeze(0)
                           for i in range(1, v.shape[0]+1)], 0)
        nmk_fact = k_fact.flip(0)
        binomial_coefficient = n_fact/(k_fact * nmk_fact)
    binomial_coefficient = binomial_coefficient.unsqueeze(
        -1).expand(N, M).double()
    range_ = torch.arange(
        N, device=sigma.device).unsqueeze(-1).expand(N, M).double()
    ones_ = torch.ones(
        N, device=sigma.device).unsqueeze(-1).expand(N, M).double()
    ...
    logv = torch.log(surface(N-1) * ZETA_CST *
                     sigma * r.sum(0) * (1/(2**(N-1))))
    logv.sum().backward()
    log_grad = sigma.grad
...
class ZetaPhiStorage(object):
    ...
    def __init__(self, sigma, N):
        ...
        if (c1 or c2 or c3):
            ...
            limit_nf = ((self.m_zeta_var/self.m_zeta_var)
                        * 0).nonzero()[0].item()
            ...
            print("\t Number of possible variance is now : " +
                  str(len(self.sigma))+"/"+str(max_nf))

        self.phi_inv_var = (
            self.sigma**3 * log_grad_zeta(self.sigma, N)).detach()
...
class CategoricalDistributionSampler(object):
    def __init__(self, distribution, n=100):
        ...
        for i, p in enumerate(self.distribution_prob):
            if (p*len(distribution) * n > 1):
                self.sampling_table.append(
                    (torch.ones(int(p*len(distribution) * n)) * i).long())
    ...
    def sample(self, sample_shape=(1,)):
        rand_index = (torch.rand(
            sample_shape, device=self.sampling_table.device).view(-1) * len(self.sampling_table)).long()

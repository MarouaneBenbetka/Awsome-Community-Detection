...
from utils.functions_utils import function as f
...
class PoincareMLR(PoincareNN):
    ...
    def forward(self, x):
        ...
        if (llp.sum() != llp.sum()):
            print("te", llp.mean(0))
        logit = norm_a * px_dot_a.sign() * f.arc_sinh((2 * px_dot_a.abs())
                                                      * lambda_px * (1/norm_a))
...
def poincareMLR_test():
    ...
    optimizer_hyperbolic = ph.PoincareBallSGDExp(
        model.poincare_parameters(), lr=1e-3)

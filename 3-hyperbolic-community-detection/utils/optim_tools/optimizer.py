...
class PoincareBallSGDAdd(PoincareOptimizer):
    ...
    def _optimization_method(self, p, d_p, lr):
        p.copy_(pf.add(pf.renorm_projection(p.data), -lr *
                pf.exp(d_p.new(d_p.size()).zero_(), d_p)))
...
class PoincareRAdam(PoincareOptimizer):
    ...
    def _optimization_method(self, p, d_p, lr):
        with torch.no_grad():
            ...
            self.v[mask] =\
                self.beta_2 * v + (1 - self.beta_2) * \
                pf.norm(p[mask], gradient)
            self.v[mask] = torch.cat(
                (v.unsqueeze(0), self.v[mask].unsqueeze(0))).max(0)[0]

            updated_weight = pf.exp(
                p[mask], - lr * m / torch.sqrt(self.v[mask]))
            self.tau[mask] =\

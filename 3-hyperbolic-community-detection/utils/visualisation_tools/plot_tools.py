...
def plot_poincare_gmm(z, gmm, labels=None, n_estim=100, marker='.',
    ...
    for z_index in range(len(Z)):
        ...
        point = torch.cat((torch.FloatTensor(
            X[z_index]).unsqueeze(-1), torch.FloatTensor(Y[z_index]).unsqueeze(-1)), -1)
        # print(point.shape)
    ...
    ax.plot_surface(X, Y, Z, rstride=1, cstride=1, linewidth=1,
                    antialiased=True, cmap=plt.get_cmap("viridis"))
    z_circle = -0.8
    ...
    for q in range(len(z)):
        c_color = [plt_colors.hsv_to_rgb(
            [float(labels[q][0])/(n_cluster), 0.5, 0.8])] if (labels is not None) else "C0"

        normalized_color = np.clip(c_color, 0, 1)
        ax.scatter(z[q][0].item(), z[q][1].item(), z_circle,
                   c=normalized_color, marker=marker, s=marker_size)

    mu = gmm._mu
    for j in range(len(mu)):
        ax.scatter(mu[j][0].item(), mu[j][1].item(),
                   z_circle, c='r', marker='D')
...
def plot_poincare_disc_embeddings(z, labels=None, centroids=None, save_folder=".", file_name="default.png",
    ...
    fig = plt.figure(" Embeddings ", figsize=(5, 5))
    ...
    for q in range(len(z)):
        ...
        c_color = [plt_colors.hsv_to_rgb(
            [float(labels[q][0])/(n_cluster), 0.5, 0.8])] if (labels is not None) else "C0"
        normalized_color = c_color / np.max(c_color)
        plt.scatter(z[q][0].item(), z[q][1].item(),
                    c=normalized_color, marker=marker, s=s)
    if (centroids is not None):
        plt.scatter(centroids[:, 0], centroids[:, 1],
                    marker='D', s=800., c='red')
...
def plot_geodesic(from_point, to_point, manifold, ax=None):
    ...
    points = []
    for f in factors:
        points.append(manifold.riemannian_exp(from_point, f *
                      manifold.riemannian_log(from_point, to_point)))
...
def test_geodesics():
    ...
    plot_poincare_disc_embeddings(
        torch.cat((from_point, to_point), 0), close=False)

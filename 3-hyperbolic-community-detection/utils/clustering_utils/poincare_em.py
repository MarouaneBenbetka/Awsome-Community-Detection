...
from utils.clustering_utils import poincare_kmeans as kmh
from utils.functions_utils import distribution_function as df
from utils.functions_utils import poincare_function as pf
from utils.functions_utils import poincare_alg as pa
...
class PoincareEM(object):
    ...
    def update_mu(self, z, wik, lr_mu, tau_mu, g_index=-1, max_iter=150):
        ...
        if (M > 40):
            ...
            if (g_index >= 0):
                self._mu[g_index] = pa.barycenter(
                    z, wik[:, g_index], lr_mu, tau_mu, max_iter=max_iter, normed=True).squeeze()
            else:
                self._mu = pa.barycenter(z.unsqueeze(1).expand(
                    N, M, D), wik, lr_mu,  tau_mu, max_iter=max_iter, normed=True).squeeze()

    def update_sigma(self, z, wik, g_index=-1):
        with torch.no_grad():
            N, D, M = z.shape + (self._mu.shape[0],)
            if (g_index > 0):
                dtm = ((self._distance(z, self._mu[:, g_index].expand(
                    N))**2) * wik[:, g_index]).sum()/wik[:, g_index].sum()
                self._sigma[:, g_index] = self.phi(dtm)
            else:
                dtm = ((self._distance(z.unsqueeze(1).expand(N, M, D), self._mu.unsqueeze(
                    0).expand(N, M, D))**2) * wik).sum(0)/wik.sum(0)
    ...
    def fit(self, z, max_iter=200, lr_mu=5e-2, tau_mu=1e-4, Y=None):
        ...
        if (not self._started):
            ...
            if (self._verbose):
                print("size", z.size())
            self.zeta_phi = df.ZetaPhiStorage(
                torch.arange(1e-2, 2., 0.001), self._d)
        ...
        if (Y is not None):
            ...
            self._n_g = old_ng
            self._mu_reshaped = torch.zeros(
                self._n_g, self._mu.size(-1)).to(z.device)
    ...
    def set_parameters(self, pi, mu, sigma):
        self._d = mu.size(-1)
        self.zeta_phi = df.ZetaPhiStorage(
            torch.arange(5e-3, 2., 0.001), self._d)
    ...
    def get_pik(self, z):
        ...
        if (p_pdf.sum(-1).min() == 0):
            print("EXPECTATION (function get_pik) : pdf.sum(-1) contain zero for ",
                  (p_pdf.sum(-1) <= 1e-15).sum().item(), "items")

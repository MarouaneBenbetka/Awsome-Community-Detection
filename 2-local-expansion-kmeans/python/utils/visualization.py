...
def _position_nodes(G, partition, **kwargs):
    ...
    for c_i, nodes in clusters.items():
        ...
        if "pos" in kwargs:
            init_pos = {n: kwargs["pos"][n]
                        for n in nodes if n in kwargs["pos"]}
            ...
            pos_subgraph = nx.spring_layout(
                subgraph, **{**kwargs, **{"scale": SUBGRAPH_SCALE}})
...
def _pos_endpoints(G, frames, seed):
    ...
    for i in range(len(frames) - 1):
        partition = frames[i]["labels"]
        next_partition = frames[i + 1]["labels"]
        ...
        mid_pos_nodes = _position_nodes(
            G, partition, pos=init_pos_nodes, seed=seed)  # partition
        mid_pos = cluster_layout(G, mid_pos_nodes, _prev_pos_clusters)

        target_pos_nodes = _position_nodes(
            G, partition, pos=mid_pos_nodes, fixed=None, seed=seed)  # partition
        target_pos_clusters = _position_clusters(G, partition)  # partition

        target_pos = cluster_layout(
            G,
            target_pos_nodes,
            {i: target_pos_clusters[c_i]
                for i, c_i in enumerate(next_partition)}
        )
...
def _transition_lengths(frames, pos_endpoints):
    ...
    trans_lengths = ((norm_D[i], norm_D[i + 1])
                     for i in range(0, len(norm_D), 2))
...
def _interpolate_frames(G, base_frame, source_pos, target_pos, interpol_len):
    for j in range(interpol_len + 1):
        ...
        for n in range(G.number_of_nodes()):
            # TODO : modifyi remove +1
            frame["pos"][n] = source_pos[n] * (1 - j / (interpol_len + 1))
            frame["pos"][n] += target_pos[n] * j / (interpol_len + 1)
...
def interpolate(G, frames, seed):
    ...
    for i, (source_pos, mid_pos, target_pos), (mid_len, targ_len) in _iter_batch:
        ...
        target_frames = _interpolate_frames(
            G,
            {**frames[i], **{"labels": frames[i + 1]["labels"], "index": i}},
            mid_pos,
            target_pos,
            targ_len
        )
...
class Animation(object):
    def __init__(self, A, frames, seed=2, dark=True):
        ...
        self.y = [frame["Modularity"] for frame in frames]
    ...
    def _calculate_axes_limits(self, node_size, node_border_size):
        ...
        xy_coords = self.ax0.transAxes.inverted().transform(
            np.vstack([xlim_pix, ylim_pix]).T)
    ...
    def init_fig(self):
        ...
        ax0_title = self.ax0.text(
            s="", transform=self.ax0.transAxes, **text_args)
        ...
        self.artists = Artists(
            nx.draw_networkx_nodes(
                ...
                node_color=self.interpolated_frames[0]["labels"],
    ...
    def update(self, i):
        ...
        Q = self.interpolated_frames[i]["Modularity"]
        partition = self.interpolated_frames[i]["labels"]
...
def clusters_animation(adj_matrix, frames, dark=False, duration=15, filename=None, dpi=None, seed=2):
...
def custom_df_styling(trace):
    df = pd.DataFrame(trace)

    df.drop(columns=['communities', 'labels'], inplace=True)

    best_nmi_index = np.argmax(df['NMI'])
    best_k_index = np.argmax(df['Modularity'])

    def highlight_row(s, row_index1, row_index2, bg_color, text_color, bg_color2, text_color2):
        ...
        for i in range(len(s)):
            ...
            if i == row_index1:
                res.append(
                    f'background-color: {bg_color} ; color : {text_color} ; font-size : 600 ')
            elif i == row_index2:
                res.append(
                    f'background-color: {bg_color2} ; color : {text_color2} ; font-size : 600 ')
            else:
                res.append('')

        return res
    styled_df = df.style.apply(highlight_row, row_index1=best_k_index, row_index2=best_nmi_index,
                               bg_color='#A7E0E0', text_color="black", bg_color2='#E8AABE', text_color2='black', axis=0)
